...
class Decoder(tf.keras.Model):
    def __init__(
        ...
        for i, neurons in enumerate(self.output_neurons):
            layer_dense = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(
                neurons,
                activation=self.activation,
                kernel_initializer=self.kernel_initializer,
            ))
    ...
    def random_feature_mask(self,X_decoder):
        ...
        feature_idx = tf.random.uniform(shape=(batch_size, output_timesteps),minval=0,maxval=num_of_features - 1,dtype=tf.int32)
        ...
        new_values = tf.random.normal(shape=(batch_size, output_timesteps), mean=0.0, stddev=1.0, dtype=tf.float32)
